...
class Sonarr:
    def __init__(self, apiKey):
        ...
        self.sonarrPort = os.environ.get('SONARR_PORT', '8989')

    def get_all_series(self, series_name):
        series_name = series_name.replace(" ", "+")
        r = requests.get(f"http://{self.sonarrIP}:{self.sonarrPort}/api/series/lookup?term={series_name}&apikey={self.apiKey}")
        return r.json()

    def get_series(self, tvdbId):
        r = requests.get(f"http://{self.sonarrIP}:{self.sonarrPort}/api/series/lookup?term=tvdb:{tvdbId}&apikey={self.apiKey}")
        return r.json()

    def get_collection(self):
        r = requests.get(f"http://{self.sonarrIP}:{self.sonarrPort}/api/series?apikey={self.apiKey}")
        return r.json()
    ...
    def add_series(self, tvdbId, seasons):
        ...
        r = requests.post(f"http://{self.sonarrIP}:{self.sonarrPort}/api/series?apikey={self.apiKey}", json.dumps(series_info))
        if r.status_code in [200, 201, 202]:
            ...
            for series in my_collection:
                if series['tvdbId'] == tvdbId:
                    series_info = series
                    series_info['seasons'] = seasons
                    r = requests.put(f"http://{self.sonarrIP}:{self.sonarrPort}/api/series?apikey={self.apiKey}", json.dumps(series_info))
                    if r.status_code in [200, 201, 202]:
                        return "EXISTS"
